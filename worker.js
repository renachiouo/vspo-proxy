    ...
    async processAndStoreVideos(videoIds, db, type, options = {}) {
        ...
        for (const videoId of videoIds) {
            ...
            if (durationMinutes <= 3.5) {
                const existingType = existingTypeMap.get(videoId);
                // Skip expensive network check if we already classified this video
                if (existingType === 'short' || existingType === 'video') {
                    doc.videoType = existingType;
                } else {
...
async function startWorker() {
    ...
    // 1. Live Status Loop (Every 5 mins)
    const runLiveCheck = async () => {
        try {
            console.log('[Worker] Running Live Status Check...');
            await v12_logic.updateLiveStatus(db);
        } catch (e) { console.error('[Worker] Live Status Error:', e); }
...
/* 
// --- Legacy Vercel Handler (Commented Out) ---
    const authenticate = () => {
