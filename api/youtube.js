...
// --- 版本指紋 ---
const SCRIPT_VERSION = '15.0-AdminUI-Backend'; 

// --- Redis Keys Configuration ---
// v4 使用 Redis 儲存動態白名單、黑名單和待審核列表
...
// --- YouTube API 設定 ---
// 【注意】以下列表僅用於「初始化」至 Redis，實際運作將讀取 Redis 中的列表
...
const v11_logic = {
    ...
    // 【修改】processAndStoreVideos 現在需要傳入 blacklist
    ...
    async updateAndStoreYouTubeData(redisClient) {
        console.log(`[v15] 開始執行中文影片常規更新程序 (自動白名單)...`);
        ...
        console.log(`[v15] 從 Redis 載入 ${currentWhitelist.length} 個中文白名單頻道和 ${blacklist.length} 個黑名單頻道。`);
        ...
        if (searchResultVideoIds.size > 0) {
            ...
            if (newChannelsToAdd.size > 0) {
                ...
                console.log(`[v15] 自動新增 ${newChannelsToAdd.size} 個新頻道到中文白名單: ${[...newChannelsToAdd].join(', ')}`);
            }
        }
        ...
        await pipeline.exec();
        console.log(`[v15] 中文影片常規更新程序完成。`);
    },
    async updateForeignClips(redisClient) {
        console.log('[v15] 開始執行外文影片常規更新程序...');
        ...
        console.log(`[v15] 從 Redis 載入 ${currentWhitelist.length} 個日文白名單頻道和 ${blacklist.length} 個黑名單頻道。`);
        
        if (currentWhitelist.length === 0) {
            console.warn("[v15] 日文白名單為空，更新程序終止。");
            return;
        }
    ...
    try {
        ...
        if (path === '/api/discover-jp-channels') {
            ...
            if (newChannelsToAdd.length > 0) {
                ...
                console.log(`[v15] 發現 ${newChannelsToAdd.length} 個新頻道，已加入待審核列表。`);
        ...
        if (path === '/api/admin/manage') {
            ...
            // 在 POST 請求中，密碼可能在 body 裡，需要先解析 body
            let body = {};
            try { body = await request.json(); } catch(e) { return response.status(400).json({ error: '無效的 JSON body' }); }
            ...
            switch(action) {
                ...
                case 'add': {
                    if (!listType || !['cn', 'jp'].includes(listType)) return response.status(400).json({ error: '新增操作需要有效的 listType (cn/jp)' });
                    await redisClient.sAdd(listType === 'cn' ? V12_WHITELIST_CN_KEY : V12_WHITELIST_JP_KEY, channelId);
                    break;
                }
                default:
        ...
        if (clientVersion && parseFloat(clientVersion.replace('V', '')) >= 11.0) {
            if (path === '/api/youtube') {
                try {
                    ...
                    if (forceRefresh) {
                        ...
                        if (needsUpdate) {
                            ...
                            if (lockAcquired) {
                                try {
                                    console.log(`[v15] 執行${isForeign ? '日文' : '中文'}影片同步更新...`);
                                    if (isForeign) {
                                        await v11_logic.updateForeignClips(redisClient);
                                    } else {
                                        await v11_logic.updateAndStoreYouTubeData(redisClient);
                ...
                try {
                    ...
                    videos.sort((a, b) => new Date(b.publishedAt) - new Date(a.pyblishedAt));
